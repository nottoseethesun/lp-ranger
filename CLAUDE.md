...
**Per-NFT scan windows:** the `IncreaseLiquidity`/`Collect`/`DecreaseLiquidity` history behind compound detection, HODL baselines and lifetime P&L runs once per NFT in a position's rebalance chain, so a long chain multiplies whatever the per-NFT window costs. Every such scan is bounded at both ends from the rebalance events the caller already holds — no extra RPC: `newTokenId` + `blockNumber` gives the mint block (lower), `oldTokenId` + `blockNumber` gives the block its replacement was minted (upper). A retired NFT stops emitting there, and the app never returns to a drained NFT, so only the CURRENT NFT scans to head. `src/nft-mint-blocks.js` owns the rule (`mintBlocksByTokenId`, `retirementBlocksByTokenId`, `nftScanFrom`, `nftScanTo`, `chainScanFloor`, `nftScanWindow`). Three rules decide correctness: the duplicate rule is asymmetric (mint map keeps the EARLIEST block, retirement map the LATEST — a bound inside an NFT's event range drops those events and the caller reads the short result as "never fired"); `nftScanFrom` combines with `Math.max`, so a mint block tightens a pool floor and a resume checkpoint beats an earlier mint block; and the chain's oldest NFT has no mint block in the events (it appears only as an `oldTokenId`) so it falls back to `chainScanFloor`, which lifts the pool's creation block to `events.firstMintBlockNumber`. On a 132-rebalance chain in a pool two years older than the first deposit: 1,144 chunks per NFT for the pool's window, 201 with the lower bound, 1-2 with both. Five files scan a chain of NFTs; `test/nft-scan-floor-coverage.test.js` fails CI for a sixth that ships unbounded, matching both on helper name and on the shape of a chunked scan whose label names a tokenId. Full detail in docs/engineering.md § "Per-NFT Scan Windows".
