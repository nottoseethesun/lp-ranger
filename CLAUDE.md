...
**Chunked log queries:** all `eth_getLogs` / `queryFilter` calls go through `src/get-logs-chunked.js`, which splits a block range into windows no wider than `getLogsChunkSize` (default 7,500 — 75% of the strictest cap observed, 10,000 on g4mm4). Endpoints reject over-wide queries with JSON-RPC `-32602`; ethers reports that as a generic `UNKNOWN_ERROR` with the real code nested, so `isBlockRangeCapError` detects it by message and the chunker raises an error naming the span, the cap and the setting to change, instead of the raw ethers dump that used to reach the Activity Log. Chunk failures propagate by default — `bestEffort` is opt-in per call site, because a swallowed cap rejection reads as "no events", which is how the original bug stayed invisible.
...
**Per-NFT scan windows:** the `IncreaseLiquidity`/`Collect`/`DecreaseLiquidity` history behind compound detection, HODL baselines and lifetime P&L runs once per NFT in a position's rebalance chain, so a long chain multiplies whatever the per-NFT window costs. Every such scan is bounded at both ends from the rebalance events the caller already holds — no extra RPC: `newTokenId` + `blockNumber` gives the mint block (lower), `oldTokenId` + `blockNumber` gives the block its replacement was minted (upper). A retired NFT stops emitting there, and the app never returns to a drained NFT, so only the CURRENT NFT scans to head. `src/nft-mint-blocks.js` owns the rule (`mintBlocksByTokenId`, `retirementBlocksByTokenId`, `nftScanFrom`, `nftScanTo`, `chainScanFloor`). Three load-bearing details: the duplicate rule is asymmetric (mint map keeps the EARLIEST block, retirement map the LATEST — a lower bound too high or an upper bound too low silently loses events); `nftScanFrom` combines with `Math.max` so an incremental resume checkpoint beats an earlier mint block; and the chain's oldest NFT has no mint block in the events (it appears only as an `oldTokenId`) so it falls back to `chainScanFloor`, which lifts the pool's creation block to the chain's own first mint. Measured on a 132-rebalance chain in a pool two years older than the first deposit: 1,144 chunks per NFT unbounded, 201 with the lower bound, 1-2 with both. Four call sites made this mistake independently; `test/nft-scan-floor-coverage.test.js` fails CI for a fifth that ships unbounded. Full detail in docs/engineering.md § "Per-NFT Scan Windows".

**Dashboard defers to the bot on managed positions:** on a cold page load the browser is asked whether a position is managed before it can answer — `isPositionManaged()` reads a Set filled from `/api/status` polling, and the bot starts positions on a stagger — so the unmanaged-details path used to scan a chain the bot was about to scan itself, two full passes per startup. `shouldSkipUnmanagedFetch()` (`public/dashboard-unmanaged.js`), consulted from `flushPendingUnmanagedFetch()`, suppresses the fetch only when a poll HAS landed and the position is managed. The `hasPolled` half is load-bearing: the managed Set is also restored from localStorage for instant badge render, so before a response arrives it may be a stale carry-over, and suppressing on it would leave a genuinely unmanaged position with nothing to populate its KPIs. Once a poll lands the answer is authoritative — the server's `managedPositions` is the union of live bot loops AND positions whose saved status is `running` but have not started yet.
