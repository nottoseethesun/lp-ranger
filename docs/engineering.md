...
- `PORT` — HTTP port (default: `5555`). The CORS origin guard is locked to
  ...
  allowed origin. See `docs/disclosure/disclosure.txt` §9 for details.
- `HOST` — Bind address (default: `127.0.0.1`, localhost only). Set to
  `0.0.0.0` for LAN access.

### Request Security

Three layers protect mutating API endpoints (POST, DELETE):

1. **Network binding** — server binds to `127.0.0.1` by default, rejecting
   non-local connections at the OS level.
2. **CORS origin guard** (`src/server-cors.js`) — rejects browser requests
   whose `Origin` header doesn't match `http://localhost:<PORT>`.
3. **CSRF tokens** (`src/server-csrf.js`) — every mutating request must
   include a valid, non-expired token in the `x-csrf-token` header. Tokens
   are generated by the `csrf` package (pillarjs), expire after 1 hour, and
   are served via `GET /api/csrf-token`. The dashboard fetches a token on
   init and auto-refreshes before expiry. Programmatic callers (e.g.
   `scripts/stop.sh`) must fetch a token before sending a POST.

GET requests (status polling, static files) require none of the above.
