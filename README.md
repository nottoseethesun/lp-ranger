...
| [Per-Token Slippage Settings](docs/roadmap/nice-to-haves/project_per_token_slippage.md) | Add per-token slippage overrides so asymmetric-liquidity pairs (e.g. `$texan / $wPls`) can carry a high slippage on the thin side without opening the deep side up to MEV front-running from an oversized budget. Unset per-token overrides fall back to the shipped `slippagePct` default (currently 0.75%). Purely additive; existing saved `slippagePct` values keep working with no migration. |
