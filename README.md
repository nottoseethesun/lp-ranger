...
The dashboard remains at <http://localhost:5555>. Note that re-syncing takes roughly **three minutes for every rebalance in a position's history**, plus a few minutes for the pool itself. A position that has not rebalanced much is ready in well under an hour; one that has rebalanced a hundred times or more can run for several hours. You can just go away and come back later to check if the "Syncing" badge at top right of the app has turned green and says, "Synced". That's when the app is ready to use.
...
- **Updating from LP Ranger version 0.9.1 or earlier** &mdash; run **Reload Current Position** once for every position you manage. This may take some time &mdash; roughly three minutes per rebalance in that position's history, so minutes for a quiet position and hours for one with a long rebalance chain.
...
| [Batch And Cache The Per-NFT History Walk](docs/roadmap/nice-to-haves/project_cache_per_nft_walk.md)                                | A position that has rebalanced many times is a chain of NFTs, and LP Ranger walks each one separately on startup over block ranges that almost entirely overlap. Measured on a 133-NFT chain: 53,268 paced queries, about three and a quarter hours. The three history events are all indexed by token id and a topic slot accepts an OR-array, so one batched pass covers the whole chain in 510 queries, about two minutes, verified against all three shipped endpoints. Caching those results against a high-water block then takes a restart to roughly seven seconds. Large payoff; the batching alone needs no cache and no new state.                                             |
