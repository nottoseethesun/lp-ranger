...
const {
  updatePositionState, createPerPositionBotState,
  getAllPositionBotStates,
} = require('../src/server-positions');
const {
  getPositionConfig, compositeKey, readConfigValue, addManagedPosition,
...
function mockPositionMgr() {
  const migrated = [];
  return {
    migrateKey(oldKey, newKey, newTokenId) { migrated.push({ oldKey, newKey, newTokenId }); },
    getMigrated() { return migrated; },
  };
}
...
describe('key migration on rebalance', () => {
  it('keyRef.current updates so subsequent calls use the new key', () => {
    ...
    const posBotState = createPerPositionBotState(diskConfig.global, getPositionConfig(diskConfig, oldKey));

    // Simulate what the server does: create keyRef + closures
    const keyRef = { current: oldKey };
    const updateBotState = (patch) => updatePositionState(keyRef, patch, diskConfig, mgr);
    ...
    updateBotState({ rebalanceError: 'test error', rebalancePaused: true });
    ...
    assert.strictEqual(getConfig('slippagePct'), 2, 'getConfig should read from migrated position config');
