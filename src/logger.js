...
function emojiId(str) { const h = crypto.createHash('md5').update(String(str)).digest(); return '\x1b[48;2;0;0;0m' + _EMOJI[h[0] % 16] + _EMOJI[h[1] % 16] + _EMOJI[h[2] % 16] + '\x1b[0m'; }
...
const HIGHLIGHTS = [
  ...
  { text: 'Manual rebalance',    style: '\x1b[38;2;60;60;60;48;2;255;140;0m', toEnd: true }, // dark grey on chevrolet orange (rest of line)
  { text: 'Rebalance OK',       style: '\x1b[48;2;0;60;120m', toEnd: true }, // light purple text (from [bot] prefix) on deep azure bg
  { text: 'Position selected:', style: '\x1b[38;2;80;40;0;48;2;124;252;0m', toEnd: true }, // dark brown text on lawn green bg
];
...
function _colorize(args) {
  ...
  for (const h of HIGHLIGHTS) {
    ...
    if (idx >= 0) {
      if (h.toEnd) { args[0] = args[0].slice(0, idx) + h.style + args[0].slice(idx) + RESET; }
      else { args[0] = args[0].replace(h.text, h.style + h.text + RESET); }
      break;
    }
  }
  return args;
